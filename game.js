window.onload = () => {
  ...
  function checkWin() {
    ...
    console.log(fetchData(letterCollected));
  }

  function fetchData(letterCollected) {
    const key = "6669422e-7427-45f8-9a37-1a52cd589c10";
    const url = `https://www.dictionaryapi.com/api/v3/references/collegiate/json/${letterCollected}?key=${key}`;

    return fetch(url)
      .then(res => res.json())
      .then(res => {
        // promiseValue = res[0].meta.id;
